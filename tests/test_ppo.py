...
<text x="{left}" y="18" font-size="16" font-family="monospace" fill="#111">PPO reward curve</text>
...
<polyline fill="none" stroke="#d62728" stroke-width="2" points="{" ".join(points)}"/>
</svg>
""",
        encoding="utf-8",
    )
...
def test_ppo_pendulum_smoke():
    ...
    for episode in range(train_episodes):
        ...
        loss_text = "n/a" if losses is None else f"actor={losses[0]:.4f} | critic={losses[1]:.4f}"
        print(f"train {episode + 1:03d}/{train_episodes} | reward={total_reward:7.1f} | {loss_text}")
    ...
    svg_path = outputs_dir / "ppo_rewards.svg"
    video_path = outputs_dir / "ppo_pendulum_demo.mp4"
